...
After the CQL engine in the `cql-on-omop` component finishes, it invokes the trigger endpoint to make the results available for review and subsequent transmission to the hub.
...
5. **Confirm or Reject**:
   ...
   - Click "Reject" to discard the data.
     Nothing is deleted from the local database, only the upload is canceled.
